...
type Props = NativeStackScreenProps<RootStackParamList, typeof RootStackRoutes.CHOOSE_CITY>

const ChooseCityScreen = ({ navigation, route }: Props) => {
  const theme = useTheme();
  const [searchTerm, setSearchTerm] = React.useState('');
  const [debouncedSearchTerm] = useDebouncedState(searchTerm);
  const [selectedResource, setSelectedResource] = React.useState<City | null>(null)
  const [resourcePages, setResourcePages] = React.useState<Array<GetCitiesResponse["cities"]>>([]);
  ...
  const { data: statesResponse, isLoading, isFetching } = useGetCitiesQuery(
    {
  ...
  const resources = React.useMemo(() => {
    ...
    return resourcePages.flatMap(resourcePage => resourcePage.data.map(resource => ({
      type: "data" as const,
      ...resource
    })))
    ...
    <>
      <View style={{ flex: 1 }}>
        ...
        {isUpdatingProfile && <View style={{ marginVertical: 10 }}>
          ...
          <Text style={{ textAlign: "center", marginTop: 10 }}>Updating City</Text>
        </View>}

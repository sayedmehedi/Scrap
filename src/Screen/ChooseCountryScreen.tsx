...
import { ActivityIndicator, Button, Text, Title, useTheme } from 'react-native-paper';
import CurrentLocationBtn from './CurrentLocationBtn';

type Props = NativeStackScreenProps<RootStackParamList, typeof RootStackRoutes.CHOOSE_COUNTRY>
...
const ChooseCountryScreen = ({ navigation, route }: Props) => {
  ...
  const [countryPages, setCountryPages] = React.useState<Array<GetCountriesResponse["countries"]>>([]);
  ...
  const { data: countriesResponse, isLoading, isFetching } = useGetCountriesQuery(
    {
  ...
  const countries = React.useMemo(() => {
    ...
    return countryPages.flatMap(countryPage => countryPage.data.map(country => ({
      type: "data" as const,
      ...country
    })))
    ...
    <>
      <View style={{ flex: 1 }}>
        ...
        {isUpdatingProfile && <View style={{ marginVertical: 10 }}>
          ...
          <Text style={{ textAlign: "center", marginTop: 10 }}>Updating Country</Text>
        </View>}
        ...
        <FlatList<typeof countries[0]>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                onPress={() => {
                  setSelectedCountry(prevSelectedCountry => prevSelectedCountry?.id === item.id ? null : item)
                }}>
                <View
                  style={{
                    paddingVertical: 15,

...
import { useGetConversationsQuery, useLazyGetConversationsQuery } from '@data/laravel/services/message';
import SkeletonPlaceholder from 'react-native-skeleton-placeholder';


const ConversationListScreen = () => {
  const [getConversations, { isFetching }] = useLazyGetConversationsQuery()
  const { data: getConversationsResponse, isLoading } = useGetConversationsQuery({})
  const [converstaionPages, setConversationPages] = React.useState<Array<GetConversationsResponse["messages"]>>([]);
  const actionCreaterRef = React.useRef<ReturnType<typeof getConversations> | null>(null);
  ...
  const conversations = React.useMemo(() => {
    ...
    return converstaionPages.flatMap(conversationPage => conversationPage.data.map(conversation => ({
      type: "data" as const,
      ...conversation
    })))

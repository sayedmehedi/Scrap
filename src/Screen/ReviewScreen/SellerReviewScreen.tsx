...
import { GetSellerReviewsResponse, PaginationQueryParams, RootStackParamList, SellerReview } from '@src/types'
import { useCreateSellerReviewMutation, useGetSellerReviewsQuery, useLazyGetSellerReviewsQuery } from '@data/laravel/services/seller'
...
type Props = NativeStackScreenProps<RootStackParamList, typeof RootStackRoutes.SELLER_REVIEW>
...
export default function SellerReviewScreen({ route }: Props) {
    ...
    const [getSellerReviews, { isFetching }] = useLazyGetSellerReviewsQuery()
    const { data: sellerReviewResponse, isLoading } = useGetSellerReviewsQuery({
        sellerId: +route.params.sellerId
    })
    const actionCreaterRef = React.useRef<ReturnType<typeof getSellerReviews> | null>(null);
    const [sellerReviewPages, setSellerReviewPages] = React.useState<Array<GetSellerReviewsResponse["reviews"]>>([]);

    const [createReview, { isLoading: isCreatingReview, isSuccess, data }] = useCreateSellerReviewMutation()
    ...
    const reviews = React.useMemo(() => {
        ...
        return sellerReviewPages.flatMap(reviewPage => reviewPage.data.map(review => ({
            type: "data" as const,
            ...review
        })))
                ...
                <React.Fragment>
                    ...
                    <Card style={{ marginVertical: 20 }}>
                        <Card.Content>
                            {reviewSummaries.map(summary => (
                                <View key={summary.id} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 15 }}>
                                    ...
                                    <View style={{ flex: 1, marginRight: 10 }}>
                                        <LinearProgress color={"#FFB703"} variant={"determinate"} value={summary.progress()} />
                                    </View>
                    ...
                    <Card style={{ marginVertical: 20 }}>
                        ...
                        <Card.Actions style={{ justifyContent: 'center', paddingVertical: 30 }}>
                            <AppPrimaryButton onPress={handleSendReview} disabled={!myReview || isCreatingReview} text='Submit' />
                        </Card.Actions>
                    </Card>
                </React.Fragment>
            )}
                ...
                return (
                    <Card>
                        <Card.Title
                            ...
                            right={() => <View style={{ paddingRight: 10 }}>
                                <Rating
                                    imageSize={15}
                                    readonly={true}
                                    showRating={false}

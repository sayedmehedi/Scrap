...
type Props = NativeStackScreenProps<RootStackParamList, typeof RootStackRoutes.CHOOSE_STATE>

const ChooseStateScreen = ({ navigation, route }: Props) => {
  const theme = useTheme();
  const [searchTerm, setSearchTerm] = React.useState('');
  const [debouncedSearchTerm] = useDebouncedState(searchTerm);
  const [selectedResource, setSelectedResource] = React.useState<State | null>(null)
  const [resourcePages, setResourcePages] = React.useState<Array<GetStatesResponse["states"]>>([]);
  const [updateProfile, { isLoading: isUpdatingProfile, isSuccess: isUpdateProfileSuccess, data: updateProfileReponse }] = useUpdateProfileMutation();


  const { data: statesResponse, isLoading, isFetching } = useGetStatesQuery(
    {
  ...
  const resources = React.useMemo(() => {
    ...
    return resourcePages.flatMap(countryPage => countryPage.data.map(country => ({
      type: "data" as const,
      ...country
    })))
  ...
  React.useEffect(() => {
    if (isUpdateProfileSuccess && !!updateProfileReponse) {
      navigation.navigate(RootStackRoutes.CHOOSE_CITY, {
  ...
  return (
    <>
      <View style={{ flex: 1 }}>
        ...
        {isUpdatingProfile && <View style={{ marginVertical: 10 }}>
          <ActivityIndicator size={"small"} />
          <Text style={{ textAlign: "center", marginTop: 10 }}>Updating State</Text>
        </View>}
        ...
        <FlatList<typeof resources[0]>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                onPress={() => {
                  setSelectedResource(prevResource => prevResource?.id === item.id ? null : item)
                }}>
                <View
                  style={{
                    paddingVertical: 15,
